...
function loader.create_plugin_manager_symlinks(selected_profile, profiles)
	...
	vim.loop.fs_symlink(
		...
		root_plugin_dir .. "/" .. profile_config.plugins,
		{ dir = true }
	)
	...
	if profile_config.preconfigure then
		...
		if preconfigure_options[1] == "packer" then
			...
			elseif preconfigure_options[2] ~= "start" and preconfigure_options[2] ~= "opt" then
				log.warn(
					"Config option for packer:{opt|start} did not match the allowed values {opt|start}. Assuming packer:start..."
				)
				table.insert(preconfigure_options, "start")
			end
			...
			vim.cmd(
				"silent !git clone https://github.com/wbthomason/packer.nvim -b "
					.. branch
					.. " "
					.. root_plugin_dir
					.. "/"
		...
		elseif preconfigure_options[1] == "paq-nvim" then
			...
			elseif preconfigure_options[2] ~= "start" and preconfigure_options[2] ~= "opt" then
				log.warn(
					"Config option for paq-nvim:{opt|start} did not match the allowed values {opt|start}. Assuming paq-nvim:start..."
				)
				table.insert(preconfigure_options, "start")
			end
			...
			-- Grab packer from GitHub with all the options
			vim.cmd(
				"silent !git clone https://github.com/savq/paq-nvim -b "
					.. branch
					.. " "
					.. root_plugin_dir
					.. "/"
...
function loader.create_plugin_symlink(selected_profile, profiles)
	...
	elseif not selected[1] then
		log.error(
			"Unable to load profile with name",
			selected_profile,
